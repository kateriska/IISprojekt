...
function tile_manage_rooms(){
  if( check_rights(FACILITY_MANAGER) ){
    insert_tile("Spr�va m�stnost�", "./rooms.php");
  }
}

function tile_manage_users(){
  if( check_rights(ADMIN) ){
    insert_tile("Spr�va u�ivatel�", "./users.php");
  }
}
...
function table_all_courses(){
  ...
  $r_table = "<table id='courses'><tr><th>ID</th><th>N�zev</th><th>Typ</th><th>Cena</th></tr>";
...
function tile_create_course(){
  if( check_rights(GARANT) ){
    insert_create_tile("Zalo�it nov� kurz", "./course_create.php");
  }
}
...
function table_users(){
  ...
  $r_table = "<table id='users'><tr><th>Jm�no</th><th>Role</th><th>Email</th></tr>";
  while($row = mysqli_fetch_assoc($result)){
...
function get_modifiable_user_details($id){
  ...
  $mail = $row['email'];
  $r_str = "<h2>Upravit �daje</h2><form action=act.user_update.php method='post'>
              Jm�no:<br><input type='text' name='firstname' value='$firstname'><br>
              P��jmen�:<br><input type='text' name='lastname' value='$lastname'><br>
              Role:<br><select name='role'>
                <option value='1'$s1>Student</option>
                <option value='2'$s2>Lektor</option>
                <option value='3'$s3>Garant</option>
                <option value='4'$s4>Vedouc�</option>
                <option value='5'$s5>Administr�tor</option>
              </select><br>
              Email:<br><input type='text' name='mail' value='$mail'><br>
              <input type='hidden' name='id' value='$id'>
              <button type='submit' name='user_edit_submit'>Potvrdit zm�ny</button>
            </form>";
  echo($r_str);
}

function get_user_set_pwd($id){
  $r_str = "<h2>Zm�nit heslo u�ivatele</h2><form action=act.user_pwd_update.php method='post'>
              Nov� heslo:<br><input type='text' name='pwd'><br>
              <input type='hidden' name='id' value='$id'>
              <button type='submit' name='user_set_pwd_submit'>Potvrdit zm�nu</button>
            </form>";
  echo($r_str);
}
